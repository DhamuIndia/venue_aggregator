import { ArrowLeft, BadgeCheck, Check, Clock3, MapPin, Star, Trophy } from "lucide-react";
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1fr)_380px]">
          <div>
            ...
            <section className="py-7"><div className="flex flex-wrap items-center justify-between gap-4"><h2 className="text-xl font-semibold">Verified customer reviews</h2><span className="inline-flex items-center gap-1 text-sm font-medium text-emerald-700"><BadgeCheck size={17} /> Completed service only</span></div>{vendor.reviews.length > 0 ? <div className="mt-5 grid gap-6">{vendor.reviews.map((review) => <article className="border-l-2 border-emerald-500 pl-4" key={review.id}><div className="flex flex-wrap items-center gap-2 text-sm"><strong>{review.customerName}</strong><span className="text-muted-foreground">{review.eventType} | {review.eventDate}</span></div><div className="mt-2 flex gap-1 text-amber-400">{Array.from({ length: review.rating }, (_, index) => <Star className="fill-current" key={index} size={15} />)}</div><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">{review.comment}</p></article>)}</div> : <p className="mt-5 rounded-md border border-dashed border-border bg-white px-4 py-5 text-sm text-muted-foreground">No verified customer reviews yet.</p>}</section>
          </div>

  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4" onMouseDown={isSubmitting ? undefined : onClose}>
      <div aria-labelledby="block-date-title" aria-modal="true" className="w-full max-w-md rounded-lg bg-white p-5 shadow-xl" onMouseDown={(event) => event.stopPropagation()} role="dialog">
        ...
        <form className="mt-6 grid gap-4" onSubmit={submit}><label className="text-sm font-medium">Date<input className="mt-2 h-11 w-full rounded-md border border-border px-3 outline-none focus:border-primary" min="2026-06-22" onChange={(event) => setDate(event.target.value)} required type="date" value={date} /></label><label className="text-sm font-medium">Slot<select className="mt-2 h-11 w-full rounded-md border border-border bg-white px-3 outline-none focus:border-primary" onChange={(event) => setSlot(event.target.value as EnquirySlot)} value={slot}>{blockableSlots.map((option) => <option key={option} value={option}>{formatSlot(option)}</option>)}</select></label><label className="text-sm font-medium">Reason<input className="mt-2 h-11 w-full rounded-md border border-border px-3 outline-none focus:border-primary" onChange={(event) => setReason(event.target.value)} placeholder="Maintenance, private event, or holiday" required value={reason} /></label>{error && <p className="rounded-md bg-rose-50 px-3 py-2 text-sm text-rose-700" role="alert">{error}</p>}<button className="mt-1 inline-flex h-11 items-center justify-center gap-2 rounded-md bg-rose-600 text-sm font-semibold text-white disabled:opacity-60" disabled={isSubmitting} type="submit">{isSubmitting && <LoaderCircle className="animate-spin" size={17} />} Block date</button></form>
      </div>
    </div>
  );
}
